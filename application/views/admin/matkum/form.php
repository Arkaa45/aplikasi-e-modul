        ...
        <form action="" method="POST">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" id="kode_matkul" name="kode_matkul" required
                           placeholder="PBD01"
                           value="<?= htmlspecialchars($matkum_data->kode_matkul ?? '') ?>"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="nama_matkul" name="nama_matkul" required
                           ...
                           value="<?= htmlspecialchars($matkum_data->nama_matkul ?? '') ?>"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brown-400 focus:border-brown-400 outline-none">
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="3"
                          placeholder="Deskripsi singkat mata praktikum"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brown-400 focus:border-brown-400 outline-none"><?= htmlspecialchars($matkum_data->deskripsi ?? '') ?></textarea>
            </div>
            
            <div class="mt-4">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" <?= ($matkum_data->is_active ?? 1) ? 'checked' : '' ?>
                           class="w-4 h-4 text-brown-500 border-gray-300 rounded focus:ring-brown-400">
                    <span class="text-sm text-gray-700">Mata Praktikum Aktif</span>
                </label>
            </div>
            
            <div class="flex gap-3 mt-6 pt-6 border-t border-gray-200">
                <button type="submit" class="px-5 py-2.5 bg-brown-500 hover:bg-brown-600 text-white rounded-lg transition">
                    <?= $edit_mode ? 'Simpan Perubahan' : 'Tambah Mata Praktikum' ?>
                </button>
                <a href="<?= base_url('admin/matkum') ?>" class="px-5 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">
                    Kembali
                </a>
            </div>
        </form>
