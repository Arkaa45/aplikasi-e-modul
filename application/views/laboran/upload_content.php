...
<div class="mb-6">
    ...
    <p class="text-sm text-gray-500">
        <a href="<?= base_url('laboran') ?>" class="hover:text-brown-500">Mata Praktikum</a>
        <span class="mx-2">/</span>
        <a href="<?= base_url('laboran/matkum/' . $matkum->id) ?>" class="hover:text-brown-500">
            <?= $matkum->nama_matkul ?>
        </a>
        <span class="mx-2">/</span>
        <span>Upload
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <?php elseif ($type == 'referensi'): ?>
                    ...
                    <div class="mb-4 hidden" id="ref_link_section">
                        ...
                        <input type="url" name="link_external" class="w-full px-4 py-3 border border-gray-300 rounded-lg"
                            placeholder="https://...">
                    </div>
                    <script>
                ...
                <?php elseif ($type == 'modul'): ?>
                    <div class="mb-4">
                        ...
                        <select name="slot_number" required class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                            <?php $preselect = isset($_GET['slot']) ? (int) $_GET['slot'] : null;
                            foreach ($available_slots as $slot): ?>
                                <option value="<?= $slot ?>" <?= $slot == $preselect ? 'selected' : '' ?>>Slot
                                    <?= $slot ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
