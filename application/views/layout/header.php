...
<head>
    ...
    <style>
        ...
        /* Card hover effect */
        .card-hover:hover { box-shadow: 0 1px 3px 0 rgba(60,64,67,.3), 0 4px 8px 3px rgba(60,64,67,.15); }
    </style>
</head>
        ...
        <div class="flex items-center gap-2">
            ...
            <div class="relative" id="userDropdown">
                <button class="flex items-center gap-2 p-1 rounded-full hover:bg-gray-100" onclick="toggleDropdown()">
                    <div class="w-8 h-8 rounded-full bg-brown-400 flex items-center justify-center text-white text-sm font-medium">
                        <?= strtoupper(substr($this->session->userdata('nama') ?? 'U', 0, 1)) ?>
                    </div>
                </button>
                
                <!-- Dropdown Menu -->
                <div id="dropdownMenu" class="hidden absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 py-2">
                    ...
                    <a href="<?= base_url('auth/logout') ?>" class="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
